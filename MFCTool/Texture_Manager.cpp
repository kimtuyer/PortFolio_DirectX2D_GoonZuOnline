...
HRESULT CTexture_Manager::Insert_Texture_Manager(TEX_ID eID, const wstring & wstrFilePath, const wstring & wstrObjectKey,const wstring & wstrStateKey, const DWORD & dwCount, D3DCOLOR color)
{
	...
	if (m_mapTexture.end() == iter_find)
	{
		...
		if (FAILED(pTexture->Insert_Texture(wstrFilePath, color,wstrStateKey, dwCount)))
			goto ERR; 
	...
	else if (TEX_MULTI == eID)
	{
		...
		if (FAILED(m_mapTexture[wstrObjectKey]->Insert_Texture(wstrFilePath, color, wstrStateKey, dwCount)))
			goto ERR; 
	}
