...
KwangHwa_Scene::KwangHwa_Scene() :m_pGameObject_Manager(CGameObject_Manager::Get_Instance())
{
}

KwangHwa_Scene::~KwangHwa_Scene()
{
	Release_Scene();

}

HRESULT KwangHwa_Scene::LoadData_MAPObject(const wstring & wstrFilePath)
{
...
HRESULT KwangHwa_Scene::LoadData_TILEObject(const wstring & wstrFilePath)
{
	HANDLE hFile = CreateFile(wstrFilePath.c_str(), GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
	if (INVALID_HANDLE_VALUE == hFile)
		return E_FAIL;
...
HRESULT KwangHwa_Scene::LoadData_Object(const wstring & wstrFilePath)
{
	HANDLE hFile = CreateFile(wstrFilePath.c_str(), GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
	if (INVALID_HANDLE_VALUE == hFile)
		return E_FAIL;
...
HRESULT KwangHwa_Scene::Ready_Scene()
{
	...
	LoadData_TILEObject(L"../Data/Kwanghwa_TileData.dat");

	LoadData_MAPObject(L"../Data/Kwanghwa_ObjectData.dat");
	...
	if (FAILED(pObject->Ready_GameObject()))
		return E_FAIL;
	pObject->Set_StateKey(L"KwanghwaHP");
	...
	if (callnumber == 1)
	{
		...
		LoadData_Object(L"../Data/Kwanghwa_UnitData.dat");
...
void KwangHwa_Scene::Update_Scene()
{

	m_pGameObject_Manager->Update_GameObject_Manager();


	//CGameObject_Manager::Get_Instance()->Get_MonsterList()
	if (CGameObject_Manager::Get_Instance()->Get_MonsterList().empty())
	{

		m_pGameObject_Manager->scenenumber = 5;

	}

}

void KwangHwa_Scene::Render_Scene()
{//SeaMap1
...
void KwangHwa_Scene::Release_Scene()
{
