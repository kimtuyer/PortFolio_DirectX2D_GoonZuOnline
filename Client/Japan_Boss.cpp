...
Japan_Boss::Japan_Boss() : m_tFrame({}), m_fFPSTime(0.f), m_fShotFPSTime(0), m_szFPS(L"")
,
...
Japan_Boss::~Japan_Boss()
{
	Release_GameObject();

}

HRESULT Japan_Boss::Ready_GameObject()
{
...
void Japan_Boss::FrameMove(float fSpeed)
{
...
void Japan_Boss::Late_Update_GameObject()
{
	//FrameMove(0.5f);
	FrameMove(0.5f);
...
void Japan_Boss::Render_GameObject()
{
	//D3DXMATRIX matScale, matTrans, matWorld;

	{
		//const TEXINFO* pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_Manager(L"Terrain", L"Tile", m_vecTile[i]->byDrawID);
		pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo_Manager(L"Japan_Boss", m_tInfo.StateKey, (DWORD)m_tFrame.fFrameStart);
		if (nullptr == pTexInfo)
			return;
...
void Japan_Boss::Release_GameObject()
{
...
HRESULT Japan_Boss::Shot(D3DXVECTOR3 &vPos, D3DXVECTOR3 &GoalPos)
{
	...
	//	
	//
	//	if (1.f <= m_fShotFPSTime) //0.035 설정시 6마리 ~50fps ,4마리는 6~7fps  //8마리부턴 움직임이상현상발생
	//	{
	//
	//		m_fShotFPSTime = 0.f;
	//
...
void Japan_Boss::Change_Animation(INFO* pObject, D3DXVECTOR3 vPos)
{
	//현재 라이플병이 위치한 인덱스값
...
void Japan_Boss::Set_GoalTarget()
{
	//{
	//	m_listGameObject[MAP] = m_pGameObject_Manager->Get_ObjectList()[MAP];
	//	D3DXVECTOR3 tPos; //탑의 위치
		...
		else //공격타겟이 이미 있는경우
		{
			if (m_tInfo.iAttack_Target == ID::PLAYER)
			{
				...
				else
				{
					...
					//m_tInfo.iAttack_Target = ID::END; //새로 추적할 타깃변경
...
HRESULT Japan_Boss::LoadData_Object(const wstring & wstrFilePath)
{
